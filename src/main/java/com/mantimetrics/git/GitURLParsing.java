 ...
 */
final class GitURLParsing {

    /**
     * Prevents instantiation of the static utility class.
     */
    private GitURLParsing() {
        throw new AssertionError("Do not instantiate GitURLParsing");
    }
     ...
     */
    static GitOwnerRepoCoordinates resolve(String owner, String name, String repoUrl) {
        if (hasText(owner) && hasText(name)) {
            return new GitOwnerRepoCoordinates(owner.trim(), name.trim());
        }
        if (!hasText(repoUrl)) {
            throw new IllegalArgumentException("Either owner/name or repoUrl must be configured");
        }

        GitOwnerRepoCoordinates parsed = parse(repoUrl.trim());
        String resolvedOwner = hasText(owner) ? owner.trim() : parsed.owner();
        String resolvedName = hasText(name) ? name.trim() : parsed.name();
        return new GitOwnerRepoCoordinates(resolvedOwner, resolvedName);
    }
     ...
     */
    static GitOwnerRepoCoordinates parse(String repoUrl) {
     ...
     * @return parsed repository coordinates
     */
    private static GitOwnerRepoCoordinates parseScpLikeUrl(String repoUrl) {
     ...
     * @return parsed repository coordinates
     */
    private static GitOwnerRepoCoordinates parsePath(String rawPath, String repoUrl) {
        if (!hasText(rawPath)) {
        ...
        return new GitOwnerRepoCoordinates(owner, repo);
    }
