 ...
 */
final class JavaTreeScanner {
    private static final Logger LOG = LoggerFactory.getLogger(JavaTreeScanner.class);
     ...
     */
    ScanResult scan(Path root, Map<String, List<String>> fileToKeys) {
        long totalFiles = countJavaFiles(root);
        List<ParsedFileRappresentation> includedFiles = new ArrayList<>();
     ...
     */
    private Optional<ParsedFileRappresentation> buildParsedSourceFile(
            Path root,
            Path path,
        ...
        String relativePath = PathUtility.toRelativeSourcePath(root, path);
        try {
            return Optional.of(new ParsedFileRappresentation(
                    relativePath,
